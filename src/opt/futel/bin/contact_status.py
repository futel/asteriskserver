...
lines = [Line(line=line, extension=None, status=None) for line in lines]
# filter for lines with extension
lines = [Line(line=line.line, extension=extension_re.search(line.line), status=None)
         for line in lines]
lines = [line for line in lines if line.extension]
# extract extension
lines = [Line(line=line.line, extension=line.extension.group(1), status=None) for line in lines]
# extract status
lines = [Line(line=line.line, extension=line.extension, status=line.line.split()) for line in lines]
lines = [Line(line=line.line, extension=line.extension, status=line.status[3]) for line in lines]
